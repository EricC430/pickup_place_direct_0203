...
parser.add_argument("--set_camera", action="store_true", default=False, help="Set the camera view to follow the robot.")
...
# Force PyTorch CUDA and cuSOLVER context initialization before Isaac Sim allocates memory
try:
    import torch
    if torch.cuda.is_available():
        # Fallback to magma if possible
        if hasattr(torch.backends.cuda, "preferred_linalg_library"):
            torch.backends.cuda.preferred_linalg_library("magma")
        # Initialize CUDA context
        _ = torch.zeros(1, device="cuda")
        # Initialize cuSOLVER/cuBLAS context by performing a dummy solve
        _ = torch.linalg.solve(torch.rand(2, 2, device="cuda"), torch.rand(2, device="cuda"))
        print("[INFO] Pre-initialized PyTorch CUDA context and cuSOLVER.")
except Exception as e:
    print(f"[WARN] Failed to pre-initialize PyTorch CUDA components: {e}")
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    # [FIX] Force use of raw observations to ensure agent-side encoders are used (Observation Alignment)
    if hasattr(env_cfg, "use_raw_observations"):
        env_cfg.use_raw_observations = True
        print("[INFO] Forced use_raw_observations=True for observation alignment with trained encoders.")
    ...
    if args_cli.set_camera:
        ...
        eye_pos = (robot_root_w + torch.tensor([1.5, 1.0, 1.2], device=env.unwrapped.device)).cpu().numpy().tolist()
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            # [DEBUG] Print debug info from env if available (e.g. lifting metrics)
            # The env automatically prints to console in step() if conditions met,
            # but we need to ensure the extras are retrieved to allow the env to update its internal state if needed.
            # (In RslRlVecEnvWrapper.step, extras is the 4th return value)
