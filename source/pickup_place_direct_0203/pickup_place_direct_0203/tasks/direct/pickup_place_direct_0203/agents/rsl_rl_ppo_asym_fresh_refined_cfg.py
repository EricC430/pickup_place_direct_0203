...
class PPORunnerCfg(RslRlOnPolicyRunnerCfg):
    num_steps_per_env = 96 #24       # skrl: rollouts
    max_iterations = 1200        # skrl: timesteps=50000. (50000 / 24 ≈ 2083 iterations)
    save_interval = 250
    ...
    # # --- New: Gripper re-exploration trigger ---
    # # Resets the last layer of the actor network and noise std at this iteration
    # gripper_reset_iteration = 0 #200  # Recommend matching the curriculum iteration
    # gripper_reset_std = 1.0        # Increase exploration noise
    # gripper_reset_bias = -1.0      # -1.0 means action output is biased to -1 (fully closed gripper)
    # # ------------------------------------------
    ...
    clip_actions = 100.0     # Reverted: Previous 1.0 was too restrictive for pre-trained policy
    # clip_actions = 1.0       # Optimized for Isaac Lab PD controller (Old: 100.0)
    ...
    policy = RslRlPpoActorCriticCfg(
        class_name="VisionAsymActorCritic",
        init_noise_std=1.0,                
        actor_obs_normalization=False,      # Closed: Using internal LayerNorm in StudentActor for BC
